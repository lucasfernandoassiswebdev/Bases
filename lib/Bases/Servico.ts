...
export default abstract class Servico<T> implements IServico<T> {
     ...
     * @returns Promise<T>
     */
    public buscarPorId = async (id: number, paramName?: string, transacao?: EntityManager, relations?: any): Promise<T> => {
        return await this.repositorio.buscarPorId(id, paramName, transacao, relations);
    }
